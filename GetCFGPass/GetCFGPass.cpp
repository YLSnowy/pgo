...
namespace {
struct GetCFGPass : public ModulePass {
...
private:
	...
	void initializeForFunction(Function& F);
...
char GetCFGPass::ID = 0;

bool GetCFGPass::doInitialization(Module& M)
{
...
void GetCFGPass::initializeForFunction(Function& F) {
 //getAnalysis<LoopInfoWrapperPass>(F);//.getLoopInfo();
}

bool GetCFGPass::SearchAlgorithm(Instruction* I, LoopInfo& LI, Instruction*& Phi, SmallVector<Instruction*, 10>& Loads, SmallVector<Instruction*, 20>& Instrs, SmallVector<Instruction*, 10>& Phis)
{
	bool PhiFound = false;
...
bool GetCFGPass::IsDep(Instruction* I, LoopInfo& LI, Instruction*& Phi, SmallVector<Instruction*, 10>& DependentLoadsToCurLoad, SmallVector<Instruction*, 20>& DependentInstrsToCurLoad, SmallVector<Instruction*, 10>& Phis)
{
	bool PhiFound = false;
	Use* OperandList = I->getOperandList();
	Use* NumOfOperands = OperandList + I->getNumOperands();
...
ConstantInt* GetCFGPass::getValueAddedToIndVarInLoopIterxxx(Loop* L)
{
...
PHINode* GetCFGPass::getCanonicalishInductionVariable(Loop* L)
{
...
Value* GetCFGPass::getLoopEndCondxxx(Loop* L)
{
	SetVector<Instruction*> BBInsts;
	auto B = L->getExitingBlock();
	int count = 0;
...
CmpInst* GetCFGPass::getCompareInstrADD(Loop* L, Instruction* nextInd)
{
	SetVector<Instruction*> BBInsts;
	auto B = L->getExitingBlock();
	int count = 0;
...
CmpInst* GetCFGPass::getCompareInstrGetElememntPtr(Loop* L, Instruction* nextInd)
{
	SetVector<Instruction*> BBInsts;
	auto B = L->getExitingBlock();
	int count = 0;
...
bool GetCFGPass::CheckLoopCond(Loop* L)
{
...
Instruction* GetCFGPass::GetIncomingValue(Loop* L, llvm::Instruction* curPN)
{
	BasicBlock* H = L->getHeader();
...
ConstantInt* GetCFGPass::getValueAddedToIndVar(Loop* L, Instruction* nextInd)
{
	bool Changed = false;
...
bool GetCFGPass::InjectPrefeches(Instruction* curLoad, LoopInfo& LI, SmallVector<llvm::Instruction*, 10>& CapturedPhis, SmallVector<llvm::Instruction*, 10>& CapturedLoads, SmallVector<Instruction*, 20>& CapturedInstrs, int64_t prefetchDist, bool ItIsIndirectLoad)
{
			...
			for (auto p : CapturedPhis) {
				...
				if (curPLoop != curLoop) {
					...
					if (PEB) {
						...
						mmap->insertBefore(BBInsts[BBInsts.size() - 1]);
						fd->insertBefore(BBInsts[BBInsts.size() - 1]);
						myopen->insertBefore(BBInsts[BBInsts.size() - 1]);
						storeFd->insertBefore(BBInsts[BBInsts.size() - 1]);
						loadFd->insertBefore(BBInsts[BBInsts.size() - 1]);
						mymmap->insertBefore(BBInsts[BBInsts.size() - 1]);
						storemmap->insertBefore(BBInsts[BBInsts.size() - 1]);
						loadCloseFd->insertBefore(BBInsts[BBInsts.size() - 1]);
						myclose->insertBefore(BBInsts[BBInsts.size() - 1]);
						loadmmap->insertBefore(BBInsts[BBInsts.size() - 1]);
						getmmap->insertBefore(BBInsts[BBInsts.size() - 1]);
...
bool GetCFGPass::InjectPrefechesOnePhiPartTwo(Instruction* I, LoopInfo& LI, Instruction* Phi, SmallVector<Instruction*, 20>& DepInstrs, int64_t prefetchDist)
{
	bool done = false;
...
bool GetCFGPass::InjectPrefechesOnePhiPartOne(Instruction* I, LoopInfo& LI, SmallVector<llvm::Instruction*, 10>& Phi, SmallVector<llvm::Instruction*, 10>& CapturedLoads, SmallVector<Instruction*, 20>& DepInstrs, int64_t prefetchDist, bool ItIsIndirectLoad)
{
	bool done = false;
	Instruction* phi = nullptr;
...
bool GetCFGPass::runOnFunction(Function& F)
{
	bool modified = false;
...
bool GetCFGPass::runOnModule(Module& M)
{
	doInitialization(M);
	auto &cg = getAnalysis<CallGraphWrapperPass>();
	/* cg.dump(); */
	/* CallGraphNode *cgn = cg[M.getFunction("_Z10write_funcPViii")]; */
	// CallGraphNode *cgn = cg[M.getFunction("main")];
	// for(const auto& I : *cgn)
	// {
	// 	errs() << "call instruction path = " << I.first << "\n";
	// }
	/* cgn->dump(); */

	// 1. 构建StringRef，需要用string构建
	StringRef sr("/home/snow/PGO-CAT/profiling/results/result.txt");
	// 2. 构建raw_fd_ostream，需要用到error_code
	std::error_code ec;
	raw_fd_ostream os(sr, ec);			// 构建具体os
	// 3. 将要写的指令写到os中（重载了<<操作符）
	// os << *ii << "\n";		//ii是Instruction类型
	...
	for (Module::iterator f = M.begin(), fe = M.end(); f != fe; ++f) {
		...
		CallGraphNode *cgn = cg[M.getFunction(f->getName())];
		for(const auto& I : *cgn)
        	{
                	errs() << "call instruction path = " << I.first << " ";
			if (I.first)
			{
				Type* value_type = I.first->getType();
    				Type::TypeID type_id = value_type->getTypeID();
    				std::string data_type = "";
    				switch (type_id) {
    				   // case Type::VoidTyID: data_type = "VoidTyID";break;
    				   // case Type::HalfTyID: data_type = "HalfTyID";break;
    				   // case Type::FloatTyID: data_type = "FloatTyID";break;
    				   // case Type::DoubleTyID: data_type = "DoubleTyID";break;
    				   // case Type::FP128TyID: data_type = "FP128TyID";break;
    				   // case Type::PPC_FP128TyID: data_type = "PPC_FP128TyID";break;
    				   // case Type::X86_MMXTyID: data_type = "X86_MMXTyID";break;
    				    case Type::IntegerTyID: data_type = "IntegerTyID";break;
    				    case Type::FunctionTyID: data_type = "FunctionTyID";break;
    				    case Type::StructTyID: data_type = "StructTyID";break;
    				    case Type::ArrayTyID: data_type = "ArrayTyID"; break;
    				    case Type::PointerTyID: data_type = "PointerTyID";break;
    				    case Type::VectorTyID: data_type = "VectorTyID";break;;
			...
			os << "call instruction path = " << I.first << " ";
			if (I.second && I.second->getFunction())       
			{
				errs() << I.second << " " << I.second->getFunction()->getName()<<"\n";
				os << I.second << " " << I.second->getFunction()->getName()<<"\n";
			}
			else
			{
				errs() << "\n";
...
static RegisterPass<GetCFGPass> X("GetCFGPass", "Hello GetCFGPass", true, true);
